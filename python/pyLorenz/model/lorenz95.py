...
class Lorenz95Model(object):
    ...
    def distance_to_dimension(self, t_current_dimension):
        # return the distance to dimension current_dimension
        if np.mod(self.m_spaceDimension, 2):
            d = (self.m_spaceDimension-1) // 2
            return np.roll(np.abs(np.arange(-d, d+1)), t_current_dimension-d)
        else:
            d = self.m_spaceDimension // 2
            return np.roll(np.abs(np.arange(-d, d)), t_current_dimension-d)
